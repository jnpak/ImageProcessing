...
Mat Logarithmetic_normalization(Mat src)
{
    Mat channel[3];
    split(src, channel);
    double max = 0;
    ...
    // printf("clock_1 == %0.5f\n", (float)(clock() - clock_1)/CLOCKS_PER_SEC);
    ...
    // printf("clock_2 == %0.5f\n", (float)(clock() - clock_2)/CLOCKS_PER_SEC);
    ...
    // imshow("l_out",l_out);
    // printf("clock_3 == %0.5f\n", (float)(clock() - clock_3)/CLOCKS_PER_SEC);
    ...
    // printf("pow_clock == %0.5f\n", (float)(clock() - pow_clock)/CLOCKS_PER_SEC);
    // clock_t restoration = clock();
    ...
    // printf("restoration == %0.5f\n", (float)(clock() - restoration)/CLOCKS_PER_SEC);
    // for (int x = 0; x < L_in.rows; x++)
    // {
    //     double* result_0_data = result[0].ptr<double>(x);
    //     double* result_1_data = result[1].ptr<double>(x);
    //     double* result_2_data = result[2].ptr<double>(x);
    //     double* l_out_data = l_out.ptr<double>(x);
    //     double* I_c_0_data = I_c[0].ptr<double>(x);
    //     double* I_c_1_data = I_c[1].ptr<double>(x);
    //     double* I_c_2_data = I_c[2].ptr<double>(x);
    ...
    //     for (int y = 0; y < L_in.cols; y++)
    //     {
    //         result_0_data[y] = l_out_data[y] * pow(I_c_0_data[y], s_data[y]);
    //         result_1_data[y] = l_out_data[y] * pow(I_c_1_data[y], s_data[y]);
    //         result_2_data[y] = l_out_data[y] * pow(I_c_2_data[y], s_data[y]);
    //     }
    // }
    // printf("clock_4 == %0.5f\n", (float)(clock() - clock_4)/CLOCKS_PER_SEC);
    dst = dst * 255 ;
...
// int main()
// {
//     Mat src = imread("/home/cilab/Desktop/SPL/Input/Station/0.png", 1);
//     CV_Assert(src.data);
//     imshow("original_image", src);
//     clock_t pow_clock = clock();
//     Mat result = Logarithmetic_normalization(src);
//     printf("all == %0.5f\n", (float)(clock() - pow_clock)/CLOCKS_PER_SEC);
//     imshow("result", result);
//     imwrite("/home/cilab/Desktop/SPL/Result/Road/Tanh_Test/test.png", result);
//     moveWindow("result", 500, 500);
//     waitKey(0);
//     return 0;
// }
...
int main()
{
    Mat src = imread("/home/cilab/Desktop/figures/Table/Input.png", 1);
    CV_Assert(src.data);
    while(t_a<2.05){
        string download_path = "/home/cilab/Desktop/figures/Table/Tanh_Test/";
        string png  = ".png";
        stringstream s1;
        s1 << t_a;
        string str_int = s1.str();
        download_path.append(str_int);

        download_path.append("_");
        stringstream s2;
        s2 << sigma_w;
        str_int = s2.str();
        download_path.append(str_int);

        download_path.append(png);
        cout<<"done : "<<download_path<<endl;
        
        Mat re = Logarithmetic_normalization(src);
        waitKey(0);
        imwrite(download_path, re);
        re.release();
        t_a += 0.05;
    }
